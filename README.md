...
You will first need to have installed [**ISC Kea**](https://www.isc.org/kea/) software **with** header/development packages (so the hook can link against it) - or use [**`addon-kea-hooks`**](https://github.com/OpenNebula/addon-kea-hooks) parent project.
...
The motivation for this hook is from OpenNebula's [VNF appliance](https://docs.opennebula.io/appliances/service/vnf.html) requirement: assign the IPv4 address (via DHCP) from a MAC address value.
